...
function cloud_m_prepare($name) {	
	...
	//不检测指定的模块,如果模块在忽略列表中，则将模块名称定义为 mystudy_local_module
	if(in_array($name, ignore_modules())){$pars['module'] = "mystudy_local_module";}
	//////////////////////////////////////////////////////////////////////////
...
function _cloud_build_params() {
	...
	//使用未修改过的cloud.mod.php 的 MD5 值返回
	foreach($clients as $cli) {
		////////////////////////////////////////////////////////////////////////
		if ($cli == '/framework/model/cloud.mod.php') {
			$string .= md5_file ( IA_ROOT . '/framework/model/cloud.mod.20160725.php' );
			///////////////////////////////////////////////////////////////////
		} else {
...
function cloud_m_build($modulename, $type = '') {
	...
	//不检测指定的模块,如果模块在忽略列表中，则将模块名称定义为 mystudy_local_module
	if(in_array($modulename, ignore_modules())){$pars['module'] = "mystudy_local_module";}
	//////////////////////////////////////////////////////////////////////////
...
function cloud_t_prepare($name) {
	...
	//不检测指定的主题,如果主题在忽略列表中，则将主题名称定义为 mystudy_local_theme
	if(in_array($name, ignore_themes())){$pars['theme'] = "mystudy_local_theme";}
	//////////////////////////////////////////////////////////////////////////
...
function cloud_t_build($name) {	
	...
	//不检测指定的主题,如果主题在忽略列表中，则将主题名称定义为 mystudy_local_theme
	if(in_array($pars['theme'], ignore_themes())){$pars['theme'] = "mystudy_local_theme";}
	//////////////////////////////////////////////////////////////////////////
	...
	if(!is_error($ret)) {
		if($ret['state'] == 'warning') {
			...
			if(!empty($ret['files'])) {
				foreach($ret['files'] as $file) {
					//忽略指定的文件
					if (in_array ( $file ['path'], ignore_files() )) {
						continue;
					}
...
function cloud_extra_account() {
	//不允许传递本地公众号信息至服务器，添加参数查询条件 WHERE	1 = 2
	$data = array();
	$data['accounts'] = pdo_fetchall("SELECT name, account, original FROM ".tablename('account_wechats') . " WHERE	1 = 2 GROUP BY account");
...
function cloud_extra_module() {
	...
	//排除忽略模块,在查询时就排除
	$sql .= ' AND `name` NOT IN (\'' . implode("','", ignore_modules()) . '\')';
	/////////////////////////////
...
function cloud_extra_theme() {
	...
	//排除忽略主题,在查询时就排除
	$sql .= ' AND `name` NOT IN (\'' . implode("','", ignore_themes()) . '\')';
	/////////////////////////////
...
function cloud_module_setting_prepare($module, $binding) {
	...
	//不检测指定的模块,如果模块在忽略列表中，则将模块名称定义为 mystudy_local_module
	if(in_array($module, ignore_modules())){$module = "mystudy_local_module";}
	//////////////////////////////////////////////////////////////////////////
